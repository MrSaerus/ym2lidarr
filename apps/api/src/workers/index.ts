...
export async function runCustomMatchAll(reuseRunId?: number, opts?: { force?: boolean }) {
  // просто проксируем на существующий воркер
  const { runCustomArtistsMatch } = await import('./mbMatch.js');
  const run = reuseRunId ? { id: reuseRunId } : await startRun('custom.match.all', { phase: 'start', c_total: 0, c_done: 0, c_matched: 0 });
  if (!run) return;
  return runCustomArtistsMatch(run.id, { force: !!opts?.force });
}

// раньше было: runCustomPushAll(reuseRunId?, {force?})
export async function runCustomPushAll(reuseRunId?: number, opts?: { force?: boolean }) {
...
export async function runYandexPullAll(reuseRunId?: number) {
  const run = reuseRunId ? { id: reuseRunId } : await startRun('yandex.pull.all', { phase: 'start', a_total: 0, a_done: 0, al_total: 0, al_done: 0 });
  if (!run) return;
...
export async function runYandexMatch(target: 'artists'|'albums'|'both' = 'both', opts?: { force?: boolean; reuseRunId?: number }) {
  ...
  const run = opts?.reuseRunId ? { id: opts.reuseRunId } : await startRun(kind, { phase: 'start', a_total: 0, a_done: 0, a_matched: 0, al_total: 0, al_done: 0, al_matched: 0 });
  if (!run) return;
...
export async function runYandexPush(target: 'artists'|'albums'|'both' = 'artists', opts?: { reuseRunId?: number }) {
  ...
  const run = opts?.reuseRunId ? { id: opts.reuseRunId } : await startRun(kind, { phase: 'start', total: 0, done: 0, ok: 0, failed: 0, target });
  if (!run) return;

  if (target === 'both') {
    await runLidarrPushEx({ target: 'artists', source: 'yandex', reuseRunId: run.id, noFinalize: true, kindOverride: kind });
    await runLidarrPushEx({ target: 'albums',  source: 'yandex', reuseRunId: run.id, kindOverride: kind });
    return;
  } else {
    return runLidarrPushEx({ target, source: 'yandex', reuseRunId: run.id, kindOverride: kind });
  }
}
