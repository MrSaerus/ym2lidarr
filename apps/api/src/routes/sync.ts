...
r.post('/yandex/pull', async (req, res) => {
  ...
  try {
    ...
    lg.info('yandex pull requested', 'sync.yandex.pull.start', { override: !!override });
    ...
    runYandexPull(override, run.id).catch((e) => {
      lg.error('worker crash (yandex pull)', 'sync.yandex.pull.worker.fail', { runId: run.id, err: e?.message });
    });

    lg.info('yandex pull started', 'sync.yandex.pull.started', { runId: run.id });
    res.json({ started: true, runId: run.id });
  } catch (e: any) {
    lg.error('yandex pull request failed', 'sync.yandex.pull.fail', { err: e?.message });
...
r.post('/lidarr/pull', async (req, res) => {
  ...
  try {
    ...
    if (target) {
      ...
      lg.info('lidarr pull requested (extended)', 'sync.lidarr.pullEx.start', { target, kind });

      const run = await startRun(kind, { phase: 'start', total: 0, done: 0, albumsTotal: 0, albumsDone: 0 });
      runLidarrPullEx(target, run.id).catch((e) => {
        lg.error('worker crash (lidarr pullEx)', 'sync.lidarr.pullEx.worker.fail', { runId: run.id, err: e?.message });
      });
      lg.info('lidarr pullEx started', 'sync.lidarr.pullEx.started', { runId: run.id, target });
      return res.json({ started: true, runId: run.id, target });
    }

    lg.info('lidarr pull requested (legacy both)', 'sync.lidarr.pull.start');

    const run = await startRun('lidarr', { phase: 'start', total: 0, done: 0 });
    runLidarrPull(run.id).catch((e) => {
      lg.error('worker crash (lidarr pull)', 'sync.lidarr.pull.worker.fail', { runId: run.id, err: e?.message });
    });
    lg.info('lidarr pull started', 'sync.lidarr.pull.started', { runId: run.id, target: 'both' });
    res.json({ started: true, runId: run.id, target: 'both' });
  } catch (e: any) {
    lg.error('lidarr pull request failed', 'sync.lidarr.pull.fail', { err: e?.message });
    res.status(500).json({ ok: false, error: e?.message || String(e) });
  }
});
...
r.post('/match', async (req, res) => {
  ...
  try {
    ...
    const tgt = ['artists', 'albums', 'both'].includes(target) ? target : 'both';

    lg.info('mb match requested', 'sync.match.start', { force, target: tgt });
    ...
    runMbMatch(run.id, { force, target: tgt }).catch((e) => {
...
r.post('/lidarr', async (req, res) => {
  ...
  try {
    ...
    lg.info('lidarr push requested', 'sync.lidarr.push.start', { target: t ?? 'from-settings', source: src });

    runLidarrPush(t, src).catch((e) => {
      lg.error('worker crash (lidarr push)', 'sync.lidarr.push.worker.fail', { target: t, source: src, err: e?.message });
    });

    res.json({ started: true, target: t ?? 'from-settings', source: src });
  } catch (e: any) {
    lg.error('lidarr push request failed', 'sync.lidarr.push.fail', { err: e?.message });
    res.status(500).json({ ok: false, error: e?.message || String(e) });
  }
});
...
r.post('/runs/:id/stop', async (req, res) => {
  ...
  try {
    ...
    if (!Number.isFinite(id)) {
      lg.warn('bad run id for stop', 'sync.runs.stop.badid', { raw: req.params.id });
    ...
    if (run.status !== 'running') {
      lg.info('run already finished', 'sync.runs.stop.already', { id, status: run.status });
    ...
    lg.error('stop run failed', 'sync.runs.stop.fail', { err: e?.message });
    res.status(500).json({ ok: false, error: e?.message || String(e) });
  }
});
...
r.post('/custom/push', async (_req, res) => {
  const lg = log.child({ ctx: { reqId: (_req as any)?.reqId } });

  try {
    const force = _req.body?.force === true || String(_req.query?.force || '').toLowerCase() === 'true';
    lg.info('custom push-all requested', 'sync.custom.pushAll.start', { force });

    const run = await startRun('custom.push.all', { phase: 'start', total: 0, done: 0, ok: 0, failed: 0 });
    runCustomPushAll(run.id, { force }).catch((e) => {
      lg.error('worker crash (custom push-all)', 'sync.custom.pushAll.worker.fail', { runId: run.id, err: e?.message });
    });

    res.json({ started: true, runId: run.id, force });
  } catch (e: any) {
    lg.error('custom push-all request failed', 'sync.custom.pushAll.fail', { err: e?.message });
    res.status(500).json({ ok: false, error: e?.message || String(e) });
  }
});
...
r.post('/yandex/pull-all', async (_req, res) => {
  ...
  try {
    lg.info('yandex pull-all requested', 'sync.yandex.pullAll.start');
    ...
    runYandexPullAll(run.id).catch((e) => {
      lg.error('worker crash (yandex pull-all)', 'sync.yandex.pullAll.worker.fail', { runId: run.id, err: e?.message });
    });

    res.json({ started: true, runId: run.id });
  } catch (e: any) {
    lg.error('yandex pull-all request failed', 'sync.yandex.pullAll.fail', { err: e?.message });
    res.status(500).json({ ok: false, error: e?.message || String(e) });
  }
});
...
r.post('/yandex/push', async (req, res) => {
  ...
  try {
    ...
    lg.info('yandex push requested', 'sync.yandex.push.start', { target, kind });
    ...
    runYandexPush(target, { reuseRunId: run.id }).catch((e) => {
      lg.error('worker crash (yandex push)', 'sync.yandex.push.worker.fail', { runId: run.id, err: e?.message });
    });

    res.json({ started: true, runId: run.id, target });
  } catch (e: any) {
    lg.error('yandex push request failed', 'sync.yandex.push.fail', { err: e?.message });
    res.status(500).json({ ok: false, error: e?.message || String(e) });
  }
});
...
r.get('/runs', async (req, res) => {
  ...
  try {
    ...
    lg.info('list runs requested (sync view)', 'sync.runs.list.start', { kind, limit });
    ...
    lg.debug('runs fetched', 'sync.runs.list.done', { count: items.length });
    res.json({ ok: true, runs: items });
  } catch (e: any) {
    lg.error('list runs failed (sync view)', 'sync.runs.list.fail', { err: e?.message });
    res.status(500).json({ ok: false, error: e?.message || String(e) });
  }
});
...
r.get('/runs/:id', async (req, res) => {
  ...
  try {
    const id = Number(req.params.id);
    lg.info('get run requested (sync view)', 'sync.runs.item.start', { id });

    const run = await prisma.syncRun.findUnique({ where: { id } });
    res.json(run);

    lg.debug('get run done (sync view)', 'sync.runs.item.done', { exists: !!run });
  } catch (e: any) {
    lg.error('get run failed (sync view)', 'sync.runs.item.fail', { err: e?.message });
    res.status(500).json({ ok: false, error: e?.message || String(e) });
  }
});
...
r.get('/runs/:id/logs', async (req, res) => {
  ...
  try {
    ...
    lg.info('get run logs requested (sync view)', 'sync.runs.logs.start', { id, afterId });
    ...
    lg.debug('get run logs done (sync view)', 'sync.runs.logs.done', { count: logs.length, nextAfter: logs.length ? logs[logs.length - 1].id : afterId });
    res.json(logs);
  } catch (e: any) {
    lg.error('get run logs failed (sync view)', 'sync.runs.logs.fail', { err: e?.message });
    res.status(500).json({ ok: false, error: e?.message || String(e) });
  }
});
