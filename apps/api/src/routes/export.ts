...
r.get('/artists.json', async (_req, res) => {
  ...
  const out = rows.map((r: any) => ({ MusicBrainzId: r.mbid! }));
...
r.get('/albums.json', async (_req, res) => {
  ...
  const out = rows.map((r: any) => ({ ReleaseGroupMBID: r.rgMbid! }));
  res.setHeader('content-type', 'application/json');
  res.send(JSON.stringify(out, null, 2));
});
...
r.get('/artists.csv', async (_req, res) => {
  ...
  const body = rows.map((r: any) => `"${r.name.replace(/"/g, '""')}",${r.mbid}`).join('\n');
...
r.get('/artists.md', async (_req, res) => {
  ...
  const head = `| # | Artist | MBID |\n|---:|---|---|\n`;
  const body = rows.map((r: any, i: any) => `| ${i + 1} | ${r.name} | \`${r.mbid}\` |`).join('\n');
...
r.get('/albums.md', async (_req, res) => {
  ...
  const head = `| # | Artist | Album | Year | ReleaseGroupMBID |\n|---:|---|---|---:|---|\n`;
  const body = rows
    .map((r: any, i: any) => `| ${i + 1} | ${r.artist} | ${r.title} | ${r.year ?? ''} | \`${r.rgMbid}\` |`)
    .join('\n');
  res.setHeader('content-type', 'text/markdown; charset=utf-8');
  res.send(head + body + '\n');
});
