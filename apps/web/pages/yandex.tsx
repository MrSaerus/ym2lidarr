...
export default function YandexPage() {
    ...
    function setTargetAndUrl(t: Target) {
        ...
        updateUrl({
            ...
            pageSize: String(pageSize),
            q,
            sortByArtists, sortDirArtists,
            sortByAlbums,  sortDirAlbums,
        });
    }
    ...
    function setSortArtistsAndUrl(field: SortFieldArtists) {
        ...
        updateUrl({
            ...
            pageSize: String(pageSize),
            q,
            sortByArtists: field, sortDirArtists: dir,
            sortByAlbums,       sortDirAlbums,
        });
    }
    function setSortAlbumsAndUrl(field: SortFieldAlbums) {
        ...
        updateUrl({
            ...
            q,
            sortByArtists,        sortDirArtists,
            sortByAlbums: field,  sortDirAlbums: dir,
        });
    }

    const pageCount = Math.max(1, Math.ceil(total / pageSize));
    const headerArrow = (active: boolean) => active ? <span className="text-xs text-gray-500">{(target === 'artists' ? sortDirArtists : sortDirAlbums) === 'asc' ? '▲' : '▼'}</span> : null;
                                ...
                                <>
                                    <Th className="select-none">
                                        <button
                                            ...
                                            className="inline-flex items-center gap-1 hover:underline"
                                        >
                                            Artist — Album {headerArrow(sortByAlbums === 'title')}
                                        </button>
                                    </Th>
                                    <Th className="select-none">
                                        <button
                        ...
                        {rows.length === 0 ? (
                            ...
                            (rows as YArtistRow[]).map((r, i) => (
                                <tr key={`${r.yandexArtistId}-${i}`}>
                                    ...
                                    <Td>
                                        {r.mbid ? (
                                            <a href={r.mbUrl || '#'} target="_blank" rel="noreferrer" className="text-primary underline">
                                                {r.mbid}
                                            </a>
                                        ) : <span className="text-gray-400">—</span>}
                                    </Td>
                                    <Td className="space-x-2">
                                        <a href={r.yandexUrl} target="_blank" rel="noreferrer" className="link-chip link-chip--ym">Yandex</a>
                                        {r.mbid && <a href={r.mbUrl || '#'} target="_blank" rel="noreferrer" className="link-chip link-chip--mb">MusicBrainz</a>}
                                    </Td>
                                </tr>
                            ))
                        ) : (
                            (rows as YAlbumRow[]).map((r, i) => (
                                <tr key={`${r.yandexAlbumId}-${i}`}>
                                    <Td>{(i + 1) + (page - 1) * pageSize}</Td>
                                    <Td>{[r.artistName, r.title].filter(Boolean).join(' — ') || '—'}</Td>
                                    <Td>{r.artistName || '—'}</Td>
                                    <Td>
                                        {r.rgMbid ? (
                                            <a href={r.rgUrl || '#'} target="_blank" rel="noreferrer" className="text-primary underline">
                                                {r.rgMbid}
                                            </a>
                                        ) : <span className="text-gray-400">—</span>}
                                    </Td>
                                    <Td className="space-x-2">
                                        <a href={r.yandexUrl} target="_blank" rel="noreferrer" className="link-chip link-chip--ym">Yandex</a>
                                        {r.rgMbid && <a href={r.rgUrl || '#'} target="_blank" rel="noreferrer" className="link-chip link-chip--mb">MusicBrainz</a>}
                                    </Td>
                                </tr>
                            ))
                        )}
